...
export type Scope = "reports" | "votes";

const DEFAULTS: Record<Scope, number> = { reports: 3, votes: 20 };
const WINDOW_MS = 86_400_000;

export function limitFor(scope: Scope): number {
  const env = scope === "reports" ? process.env.REPORTS_PER_DAY : process.env.VOTES_PER_DAY;
  const n = Number(env);
